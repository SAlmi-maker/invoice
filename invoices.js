...
const JOSKA_INVOICES = (() => {
  ...
  function buildPDFPageClassic(doc, inv) {
    ...
    doc.text(`Issue: ${issueStr}`, W - M, metaY + 4.5, { align: 'right' });
    doc.text(`Due: ${dueStr}`, W - M, metaY + 9, { align: 'right' });
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(100, 116, 139);
    if (inv.cin) { doc.text(`CIN: ${inv.cin}`, billToX, y2); y2 += 4; }
    if (inv.phone) { doc.text(`Tel: ${inv.phone}`, billToX, y2); y2 += 4; }
    doc.text(`${inv.vehicleBrand || ''} ${inv.vehicleModel || ''}`.trim() || '—', billToX, y2); y2 += 4;
    if (inv.plate) { doc.text(`Plate: ${inv.plate}`, billToX, y2); y2 += 4; }
    ...
    doc.text(`${daysN} ${JOSKA_I18N.t('inv.days')}`, W - M - 6, y + 9.5, { align: 'right' });
    y += 18;
    ...
    doc.text('DESCRIPTION', colX[0] + 4, y + 5.5);
    doc.text('QTY', colX[1] + 2, y + 5.5);
    doc.text('UNIT PRICE', colX[2] + 2, y + 5.5);
    doc.text('AMOUNT', colX[3] + 8, y + 5.5);
    y += 8;

    const dailyPrice = parseFloat(inv.dailyPrice || 0);
    const rental = daysN * dailyPrice;
    const extras = [
      { label: JOSKA_I18N.t('inv.field.insurance'), val: parseFloat(inv.insurance || 0) },
      { label: JOSKA_I18N.t('inv.field.fuel'), val: parseFloat(inv.fuel || 0) },
      { label: JOSKA_I18N.t('inv.field.extraDriver'), val: parseFloat(inv.extraDriver || 0) },
      { label: JOSKA_I18N.t('inv.field.other'), val: parseFloat(inv.other || 0) },
    ];
    const activeExtras = extras.filter(e => e.val > 0);
    const rowCount = 1 + activeExtras.length;
    ...
    drawRow(`${JOSKA_I18N.t('inv.field.rentalSubtotal')} (${inv.vehicleBrand || ''} ${inv.vehicleModel || ''})`, daysN, dailyPrice, rental, 0);
    ...
    sumLines.push({ label: 'Subtotal', value: formatCurrency(subtotal, currency), bold: false });
    if (extrasTotal > 0) {
      sumLines.push({ label: 'Extras', value: formatCurrency(extrasTotal, currency), bold: false });
    }
    // Separator
    const sumStart = y;
    ...
    doc.text('GRAND TOTAL', sumX + 4, y + 4.5);
    doc.setFontSize(10);
    doc.setTextColor(255, 255, 255);
    ...
    doc.setFontSize(6.5);
    doc.setTextColor(255, 255, 255);
    doc.text(JOSKA_I18N.t(`dash.${status}`).toUpperCase(), M + 15, y - 4.5, { align: 'center' });

    // ── Footer ────────────────────────────────────────────────
    const footerY = 248;
    doc.setDrawColor(226, 232, 240);
    doc.line(M, footerY, W - M, footerY);
    const footCol = W - M - 90;
    if (inv.notes) {
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(6.5);
      doc.setTextColor(148, 163, 184);
      doc.text('NOTES', M, footerY + 6);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7);
      doc.setTextColor(100, 116, 139);
    ...
    doc.setFontSize(6.5);
    doc.setTextColor(148, 163, 184);
    doc.text('Generated by JOSKA — Invoice & Revenue Management', W / 2, 285, { align: 'center' });
